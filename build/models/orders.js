...
class OrderStore {
    ...
    async update(order) {
        try {
            const conn = await database_1.default.connect();
            const result = await conn.query("UPDATE orders SET status=COALESCE($1,status), user_id=COALESCE($2,user_id) WHERE id=$3 RETURNING *", [order.status, order.user_id, order.id]);
            conn.release();
            return result.rows[0];
        }
        catch (error) {
